...
@register("offset10-model-bn")
class Offset10ModelBN(_OffsetModel, ConvolutionalModelMixin):
    """CEBRA model with a 10 sample receptive field."""
    ...
    def __init__(self, num_neurons, num_units, num_output, normalize=True):
        ...
        super().__init__(
            nn.Conv1d(num_neurons, num_units, 2),
            nn.GELU(),
            cebra_layers._Skip(nn.Conv1d(num_units, num_units, 3),nn.BatchNorm1d(num_units), nn.GELU()),
            cebra_layers._Skip(nn.Conv1d(num_units, num_units, 3),nn.BatchNorm1d(num_units),  nn.GELU()),
            cebra_layers._Skip(nn.Conv1d(num_units, num_units, 3),nn.BatchNorm1d(num_units),  nn.GELU()),
            nn.Conv1d(num_units, num_output, 3),
            num_input=num_neurons,
...
class Offset10Modelq(_OffsetModel, ConvolutionalModelMixin):
        """CEBRA model with a 10 sample receptive field."""

        def __init__(self, num_neurons, num_units, num_output, normalize=True, sigma=2):
            if num_units < 1:
                raise ValueError(
                    f"Hidden dimension needs to be at least 1, but got {num_units}."
                )
            super().__init__(
                GaussianSmoothing(num_neurons, 20, sigma, 1),
                nn.Conv1d(num_neurons, num_units, 2),
                nn.GELU(),
                cebra_layers._Skip(nn.Conv1d(num_units, num_units, 3), nn.GELU()),
                cebra_layers._Skip(nn.Conv1d(num_units, num_units, 3), nn.GELU()),
...
@_register_conditionally("offset36-model-more-dropout-bn")
class Offset36Dropoutv2BN(_OffsetModel, ConvolutionalModelMixin):
    """CEBRA model with a 10 sample receptive field.
...
@_register_conditionally("offset36-model-more-more-dropout")
class Offset36Dropoutv205(_OffsetModel, ConvolutionalModelMixin):
    """CEBRA model with a 10 sample receptive field.
    ...
    def __init__(self, num_neurons, num_units, num_output, normalize=True):
        ...
        super().__init__(
            ...
            nn.GELU(),
            *self._make_layers(num_units, 0.5, 16),
            nn.Conv1d(num_units, num_output, 3),
            num_input=num_neurons,
            num_output=num_output,
            normalize=normalize,
...
@_register_conditionally("offset36-model-more-dropout-gauss")
class Offset36Dropoutv2q(_OffsetModel, ConvolutionalModelMixin):
    """CEBRA model with a 10 sample receptive field.
...
@_register_conditionally("offset100-model-dropout")
class Offset100Dropoutv2(_OffsetModel, ConvolutionalModelMixin):
    """CEBRA model with a 10 sample receptive field.
    ...
    def __init__(self, num_neurons, num_units, num_output, normalize=True):
        ...
        super().__init__(
            ...
            torch.nn.Dropout1d(p=0.1),
            nn.GELU(),
            *self._make_layers(num_units, 0.1, 48),
            nn.Conv1d(num_units, num_output, 3),
            num_input=num_neurons,
            num_output=num_output,
            normalize=normalize,
...
class PerTimestepLinear(nn.Module):
    ...
    def forward(self, x):  # x: (B, C, T)
        ...
        return x.transpose(1, 2)  # (B, out_ch, T)


@_register_conditionally("offset36-model-more-dropout-with-prefix-fc")
class Offset36Dropoutv2WithPrefixFC(_OffsetModel, ConvolutionalModelMixin):
    """Offset36Dropoutv2 + two prepended FC-like layers (no skipping).
    Keeps original `net.*` intact and used.
    """
    ...
    def __init__(
            ...
            num_output: int,
            *,
            fc_hidden: Optional[int] = None,  # width for the middle "FC" layer
            dropout_p: float = 0.1,
            normalize: bool = True,
    ):
        if num_units < 1:
        ...
        # Build the ORIGINAL stack as `self.net` — names and structure unchanged.
        super().__init__(
            nn.Conv1d(num_neurons, num_units, 2),
            torch.nn.Dropout1d(p=dropout_p),
            nn.GELU(),
            *self._make_layers(num_units, dropout_p, 16),
            nn.Conv1d(num_units, num_output, 3),
            num_input=num_neurons,
            num_output=num_output,
            normalize=normalize,
        ...
        # New prepended per-timestep "FC" stack (1x1 convs), ending back at num_neurons.
        self.prefix = nn.Sequential(
            # nn.Conv1d(num_neurons, fc_hidden, kernel_size=1),
            # nn.GELU(),
            # nn.Dropout1d(p=dropout_p),
            # PerTimestepLinear(num_neurons, fc_hidden),
            # nn.GELU(),
            # nn.Dropout1d(p=dropout_p),
            PerTimestepLinear(num_neurons, num_neurons),
            nn.GELU(),
            nn.Dropout1d(p=dropout_p),
        )

        self.suffix = nn.Sequential(
            nn.Linear(num_output, fc_hidden),
            nn.GELU(),
            nn.Dropout(p=dropout_p),
            nn.Linear(fc_hidden, num_output),
            nn.Dropout(p=dropout_p),
        )
    ...
    def forward(self, x):
        # x = self.prefix(x)
        x = self.net[:-2](x)
        if x.dim() == 3 and x.shape[2] != self.num_output and x.shape[1] == self.num_output:
            x = x.transpose(1, 2)
            x = self.suffix(x)
            x = x.transpose(1, 2)
        else:
    ...
    def get_offset(self) -> cebra.data.datatypes.Offset:
        # 1x1 convs do not change receptive field
        return cebra.data.Offset(18, 18)
