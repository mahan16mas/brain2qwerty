...
class TimeContrastive(abc_.JointDistribution, abc_.HasGenerator):
    ...
    def sample_conditional(self, reference_idx: torch.Tensor, trial_ids=None, trial_borders=None) -> torch.Tensor:
        """Return samples from the time-contrastive conditional distribution.

        Ensures positive pairs stay within the same trial if trial_ids or trial_borders are provided.
        """
        conditional_idx = reference_idx + self.time_offset
        # print(reference_idx)
        # print(trial_ids, trial_borders)
        if trial_ids is not None and trial_borders is not None:
            conditional_idx = conditional_idx.clone()

            trial_id = trial_ids[reference_idx]

            start = trial_borders[trial_id]
            end = trial_borders[trial_id + 1]

            conditional_idx = torch.minimum(torch.maximum(conditional_idx, start), end - 1)
