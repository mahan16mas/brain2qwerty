...
class ContinuousDataLoader(cebra_data.Loader):
    ...
    * alternatively, the distribution can be selected to be a Gaussian distribution
    ...
    def get_indices(self, num_samples: int) -> BatchIndex:
        ...
        negative_idx = reference_idx[num_samples:]
        reference_idx = reference_idx[:num_samples]
        # print(self.distribution.__class__)
        # print(self.dataset.trial_ids, self.dataset.trial_borders)
        # print(isinstance(self.distribution, cebra.distributions.TimeContrastive))
        
        if isinstance(self.distribution, cebra.distributions.TimeContrastive):
            positive_idx = self.distribution.sample_conditional(reference_idx, self.dataset.trial_ids, self.dataset.trial_borders)
        else:
