...
class Dataset(abc.ABC, cebra.io.HasDevice):
    ...
    def expand_index(self, index: torch.Tensor) -> torch.Tensor:
        ...
        if self.trial_ids is not None:
            return self.expand_index_in_trial(index, self.trial_ids, self.trial_borders)
    ...
    def expand_index_in_trial(self, index, trial_ids, trial_borders):
        """
        Vectorized version of expand_index_in_trial.
        """
        device = index.device

        trial_borders = torch.tensor(trial_borders, device=device)

        offset = torch.arange(-self.offset.left, self.offset.right, device=device)

        trial_ids = torch.tensor(trial_ids, device=device)
        trial_ids = trial_ids[index]
        trial_start = trial_borders[trial_ids] + self.offset.left
        trial_end = trial_borders[trial_ids + 1] - self.offset.right

        index_clamped = torch.clamp(index, trial_start, trial_end)

        expanded_index = index_clamped[:, None] + offset[None, :]
