...
class CEBRA(TransformerMixin, BaseEstimator):
    ...
    def __init__(
        ...
        training_mode: str = "standard",  # or "adversarial"
        adv_epsilon: float = 0.05,
        adv_alpha: float = 0.01,
        adv_steps: int = 10,
        attack_norm: str = "linf",  # or "l2"
    ):
    ...
    def _prepare_fit(
        ...
        *y,
        ...
        dataset.trial_ids=trial_ids
        dataset.trial_borders = trial_borders
        ...
        solver = cebra.solver.init(
            ...
            optimizer=optimizer,
            tqdm_on=self.verbose,
            # Additional adversarial arguments
        ...
        print(loader.dataset.trial_ids)
        print(loader.__class__)
    ...
    def _adapt_model(
        ...
        if self.model_architecture in ['offset36-model-more-dropout', 'offset36-model-more-dropout-with-prefix-fc']:
            # self.model_architecture = 'offset36-model-more-dropout-with-prefix-fc'
            # adapt_model = self._prepare_model(dataset, is_multisession)
            #
            # _ = adapt_model.load_state_dict(self.model_.state_dict(), strict=False)
            #
            # for n, p in adapt_model.named_parameters():
            #     p.requires_grad = n.startswith("prefix.") or n.startswith("suffix.")

            adapt_model = self._prepare_model(dataset, is_multisession)

            _ = adapt_model.load_state_dict(self.model_.state_dict(), strict=True)

            trained_params = list(self.model_.state_dict().keys())[2:]
    ...
    def partial_fit(
        ...
        *y,
        ...
        if not hasattr(self, "state_") or self.state_ is None:
            self.state_ = self._prepare_fit(X, *y, trial_ids=trial_ids, trial_borders=trial_borders)
    ...
    def _adapt_fit(
        ...
        *y,
        ...
        """
        self.state_ = self._adapt_model(X, *y, trial_ids=trial_ids, trial_borders=trial_borders)
        self._partial_fit(*self.state_,
                          callback=callback,
                          callback_frequency=callback_frequency)
        return self
